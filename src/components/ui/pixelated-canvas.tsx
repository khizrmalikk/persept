...
type PixelatedCanvasProps = {
  ...
  fadeSpeed?: number;
  /** Auto-animate pixels without pointer movement. */
  idleMotion?: boolean;
  /** Radius factor (0..1 of min dimension) for idle orbit. */
  idleRadius?: number;
  /** Rotations per second for idle orbit. */
  idleSpeed?: number;
};
  ...
  React.useEffect(() => {
    ...
    img.onload = () => {
      ...
      const animate = () => {
        ...
        if (idleMotion && !pointerInsideRef.current) {
          const dims = dimsRef.current;
          if (dims) {
            const cx = dims.width / 2;
            const cy = dims.height / 2;
            const r = Math.min(dims.width, dims.height) * idleRadius;
            const angle = now * 0.001 * idleSpeed * Math.PI * 2;
            targetMouseRef.current.x = cx + Math.cos(angle) * r;
            targetMouseRef.current.y = cy + Math.sin(angle) * r;
            activityTargetRef.current = 1;
          }
        }
